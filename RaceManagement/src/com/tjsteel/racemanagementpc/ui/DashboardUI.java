...
class ValueBox extends JPanel {
	...
	public static enum Layout {
		VALUE_BELOW, VALUE_RIGHT
	};

	private JLabel lblTitle;
	private JLabel lblValue;
	private float criticalLow = Float.NEGATIVE_INFINITY;
	private float criticalHigh = Float.POSITIVE_INFINITY;
	private Color criticalLowColour = Color.CYAN;
	private Color criticalHighColour = Color.RED;
	...
	public void setValue(String value) {
		...
		if (f > this.criticalHigh) {
			this.setBackground(criticalHighColour);
		} else if (f < this.criticalLow) {
			this.setBackground(criticalLowColour);
		} else {
			this.setBackground(DashboardUI.defaultBackgroundColor);
		}
	}

	public void setColour(Color background) {
		lblValue.setBackground(background);
	}

	public void setCriticalLow(float value) {
		this.criticalLow = value;
	}

	public void setCriticalHigh(float value) {
		this.criticalHigh = value;
	}

	public void setCriticalLowColour(Color colour) {
		this.criticalLowColour = colour;
	}

	public void setCriticalHighColour(Color colour) {
		this.criticalHighColour = colour;
	}
}
...
public class DashboardUI extends DefaultUI {
	...
	public void initialize() {
		...
		vbCurrentGear = new ValueBox("Current Gear", "N", 260, 397, 186, 251, 30,
				com.tjsteel.racemanagementpc.ui.ValueBox.Layout.VALUE_BELOW);
		...
		vbFuelLapsRemaining = new ValueBox("Fuel, Laps Left", "888.8", 456, 528, 120, 120, 30,
				com.tjsteel.racemanagementpc.ui.ValueBox.Layout.VALUE_BELOW);
	...
	public void updateFields() {
		...
		try {
			...
			if (currPlayer.getTelemetryPacket() != null) {
				this.vbCurrentLap
						.setValue(Calc.secondsToTime(currPlayer.getTelemetryPacket().getTrack().getCurrentLap()));
				...
				this.vbTyreTempFrontLeft
						.setValue(df1.format(currPlayer.getTelemetryPacket().getTyre().getTyreTempFrontLeft()));
				this.vbTyreTempFrontRight
						.setValue(df1.format(currPlayer.getTelemetryPacket().getTyre().getTyreTempRearLeft()));
				this.vbTyreTempRearLeft
						.setValue(df1.format(currPlayer.getTelemetryPacket().getTyre().getTyreTempFrontRight()));
				this.vbTyreTempRearRight
						.setValue(df1.format(currPlayer.getTelemetryPacket().getTyre().getTyreTempRearRight()));
				...
				this.lineTrackMap.addValue(new Point2D.Double(currPlayer.getTelemetryPacket().getTrack().getPositionX(),
						currPlayer.getTelemetryPacket().getTrack().getPositionZ()));
				...
				this.lblHandbrake
						.setText(Short.toString(currPlayer.getTelemetryPacket().getPlayerInput().getHandbrake()));
